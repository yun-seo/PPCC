...
def separate_point_cloud(xyz, num_points, crop, fixed_points = None, padding_zeros = False):
    ...
    for points in xyz:
        ...
        distance_matrix = torch.norm(center.unsqueeze(2) - points[:,:,:3].unsqueeze(1), p =2 ,dim = -1)  # 1 1 2048
...
def copy_sample(pc, n):
    idx = np.random.permutation(pc.shape[0])
    idx = np.concatenate([idx, np.random.randint(pc.shape[0], size=n-pc.shape[0])])
    return pc[idx[:n]]

def part_drop_point_cloud(xyz, num_points, mode):
    _,n,c = xyz.shape
    assert n == num_points
    INPUT = []
    for points in xyz:
        uni_part = points[...,3].unique()
        if mode == 'random':
            drop_label = uni_part[torch.randperm(len(uni_part))[0]]
        elif mode == 'leg':
            drop_label = int(2)
        input_data =  points[points[...,3] != drop_label].unsqueeze(0)
        if input_data.shape[1] < 2048:
            INPUT.append(copy_sample(input_data.squeeze(0),2048).unsqueeze(0))
        else:
            INPUT.append(fps(input_data,2048))
